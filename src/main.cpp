    ...
    std::vector<InstancePtr> &instances)
{
    ...
    ScopedIfs scoped_ifs(ifs);  //auto close
    string line;
    vector<string> flds;
    uint_t linu = 0;
...
void GetFeats(int_t argc, char **argv){
    if (argc < 3) {
        fprintf(stdout, "Fdcnn toolkit v 0.1b\n\n");
...
void test_train1() {
    char * argv[] = { \
        "", \
        "-train", "D:\\runtime\\fdcnn\\train.tsv", \
        "-model", "D:\\runtime\\fdcnn\\test_model", \
        "-config", "D:\\runtime\\fdcnn\\config.txt", \
        "-word2vec", "D:\\runtime\\fdcnn\\small", \
        "-threads", "1", \
        "-alpha", "0.01", \
        "-lambda", "0", \
        "-batch_size", "2", \
...
void test_switch_model() {
    char * argv[] = { \
        "", \
        "D:\\runtime\\45000hot_iter1.bin", "D:\\runtime\\45000hot_iter1.txt"
    };
    int_t argc = 3;
    SwitchModel(argc, argv);
}

void test_switch_model1() {
    char * argv[] = { \
        "", "", \
        "D:\\runtime\\45000hot_iter.txt", "D:\\runtime\\45000hot_iter1.bin"
    };
    int_t argc = 4;
    SwitchModel(argc, argv);
}
