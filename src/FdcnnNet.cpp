    ...
    bool isTrain)
{
    ...
    drop_out_forward(conv_M[0], ncm[0], ncr[0] * ncc[0],
        dropout_conv_mask_M[0], conv_M[0], dropout_prob, seed, isTrain);
    ...
    for (i = 1, tmp = i - 1; i < ncl; ++i, ++tmp){
        ...
        drop_out_forward(conv_M[i], ncm[i], ncr[i] * ncc[i],
            dropout_conv_mask_M[i], conv_M[i], dropout_prob, seed, isTrain);
    ...
    drop_out_forward(&(hid_V[0]), 1, nhr[0], &(dropout_hid_mask_V[0]),
        &(hid_V[0]), dropout_prob, seed, isTrain);
    ...
    for (i = 1, tmp = i - 1; i < nhl; ++i, ++tmp) {
        ...
        drop_out_forward(&(hid_V[i]), 1, nhr[i], &(dropout_hid_mask_V[i]),
            &(hid_V[i]), dropout_prob, seed, isTrain);
    }
    ...
    real_t ***diff_MM, real_t **diff_WM, real_t *diff_U)
{
    ...
    for (i = htop, tmp = i - 1; i > 0; --i, --tmp) {
        drop_out_backward(&(diff_hid_V[i]), 1, nhr[i],
    ...
    //hidden 0 to cm top
    drop_out_backward(&(diff_hid_V[0]), 1, nhr[0],
        &(dropout_hid_mask_V[0]), &(diff_hid_V[0]), dropout_prob);
    non_linear_backward(&(diff_hid_V[0]), 1, nhr[0], &(hid_V[0]),
        &(diff_hid_V[0]));
    //for (int_t tst = 0; tst < nhr[0]; ++tst) {
    //    fprintf(stderr, "%1d ", dropout_hid_mask_V[0][tst]);
    //}
    //fprintf(stderr, "\n");
    ...
    for (i = ctop, tmp = i - 1; i > 0; --i, --tmp) {
        ...
        drop_out_backward(diff_conv_M[i], ncm[i], ncr[i] * ncc[i],
    ...
    drop_out_backward(diff_conv_M[0], ncm[0], ncr[0] * ncc[0],
...
void FdcnnNet::updateParams(int_t thrd_id)
{
    ...
    for (i1 = 0; i1 < ncl; ++i1) {
        ...
        //fprintf(stdout, "diff_MM[%d]= ", i1);
        for (i2 = 0; i2 < tmp1; ++i2) {
            for (i3 = 0; i3 < tmp2; ++i3) {
                MM[i1][i2][i3] -= diff_MM[thrd_id][i1][i2][i3] * alpha
                    + lambda * MM[i1][i2][i3];
                //fprintf(stdout, "%.6f ", diff_MM[thrd_id][i1][i2][i3]);
            }
        }
        //fprintf(stdout, "\n");
    }
    for (i1 = 0; i1 < nhl; ++i1) {
        ...
        //fprintf(stdout, "diff_WM[%d]= ", i1);
        for (i2 = 0; i2 < tmp1; ++i2) {
            WM[i1][i2] -= diff_WM[thrd_id][i1][i2] * alpha
                + lambda * WM[i1][i2];
            //fprintf(stdout, "%.6f ", diff_WM[thrd_id][i1][i2]);
        }
        //fprintf(stdout, "\n");
    }
    tmp1 = nor * (nhr[nhl - 1] + 1);
    //fprintf(stdout, "diff_U= ");
    for (i1 = 0; i1 < tmp1; ++i1) {
        U[i1] -= diff_U[thrd_id][i1] * alpha + lambda * U[i1];
        //fprintf(stdout, "%.6f ", diff_U[thrd_id][i1]);
    }
    //fprintf(stdout, "\n");
    ...
    int_t thrd_id)
{
    ...
    while (true) {
        ...
        for (i1 = 0; i1 < batch_size; ++i1) {
            ...
            //fprintf(stdout, "diff_out_V= ");
            //for (i2 = 0; i2 < nor; ++i2) {
                //fprintf(stdout, "%.6f ", diff_out_V[thrd_id][i1][i2]);
            //}
            //fprintf(stdout, "\n");
            //fprintf(stdout, "out_V= ");
            //for (i2 = 0; i2 < nor; ++i2) {
                //fprintf(stdout, "%.6f ", out_V[thrd_id][i1][i2]);
            //}
            //fprintf(stdout, "\n");
    ...
    std::vector<ProbType> &probs, int_t thrd_id)
{
    ...
    Forward(sen_M[thrd_id], lsen[thrd_id], conv_M[thrd_id],
        ncc[thrd_id], dropout_conv_mask_M[thrd_id], pool_V[thrd_id],
        ndk[thrd_id], pool_M[thrd_id], hid_V[thrd_id],
        dropout_hid_mask_V[thrd_id], out_V[thrd_id], rand_seed, false);
    //calculate error item for out_V
    ...
    for (i = 0; i < lsen; ++i) {
        for (j = 0, k = 0; j < nif; k += nfd[j], ++j) {
            ...
            for (l = 0; l < nfd[j]; ++l) {
                ...
                //fprintf(stdout, "%.6f ", diff_sen_M[(l + k) * lsen + i]);
